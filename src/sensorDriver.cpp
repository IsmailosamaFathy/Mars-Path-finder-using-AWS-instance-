...
void sclass::SETUP() {
  ...
  // intial value was 32
  sensor.setFrameTiming(1024); /* to average the specified number of samples taken before returning a value */
  ...
  if (dataReady) /* when we have data to read */
  {
    ...
    if (sensor.channelUsed == 2) /* iterate only when we have read the 3 channels */
    {
      for (int16_t i = 0; i < 3; i++)
      {
        ...
        // Serial.print(i);
        // Serial.print(": ");
        // Serial.println(arr[i]); // Print the values of distances array
      }
      dataReady = false;  /* put low to restart the sampling */
      delay(10);           // initial value was 1000 then later 100
    }
  }
  return arr;
